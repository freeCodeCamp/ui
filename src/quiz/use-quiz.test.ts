...
describe("useQuiz", () => {
	...
	it("should pass the `showFeedback` prop to the answers when `validateAnswers` is called", () => {
		const { result } = renderHook(() =>
			useQuiz({
				initialQuestions: [
					{
		...
		expect(
			result.current.questions.every(
				(question) => question.showFeedback === undefined,
			),
		).toBe(true);
		...
		expect(
			result.current.questions.every(
				(question) => question.showFeedback === true,
			),
		).toBe(true);
	});
});
